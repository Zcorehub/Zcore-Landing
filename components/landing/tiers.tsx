...
const TIERS = [
  {
    ...
    glow: "hover:border-emerald-500/50 hover:shadow-[0_0_40px_rgba(16,185,129,0.12)]",
  ...
  },
  {
    ...
    glow: "hover:border-blue-500/50 hover:shadow-[0_0_40px_rgba(59,130,246,0.12)]",
  ...
  },
  {
    ...
    glow: "hover:border-yellow-500/50 hover:shadow-[0_0_40px_rgba(234,179,8,0.1)]",
    ...
    <section id="tiers" className="py-24 px-4 sm:px-6 bg-[#050810]/50">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {TIERS.map(({ tier, range, limit, rate, badge, glow, bar, label }, i) => (
            <Reveal key={tier} delay={i * 80}>
              <div
                className={cn(
                  "group h-full p-6 rounded-2xl border border-white/[0.08] bg-[#0a0e1a]/60 transition-all duration-300",
                  glow
                )}
              >
                <div
                  className={cn(
                    "inline-flex px-3 py-1 rounded-full text-xs font-black border mb-5",
                    badge
                  )}
                >
                  {label ?? `Tier ${tier}`}
                </div>
                <div className="text-3xl font-black mb-1">{range}</div>
                <div className="h-1 rounded-full bg-white/[0.06] mb-4 overflow-hidden">
                  <div
                    className={cn("h-full rounded-full bg-gradient-to-r opacity-60", bar)}
                    style={{
                      width:
                        tier === "A"
                          ? "100%"
