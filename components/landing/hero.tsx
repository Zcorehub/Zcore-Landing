...
export function Hero() {
  ...
  const motionStyle = (delay: number) =>
    reducedMotion ? undefined : fadeUp(delay);
  const entranceClass = (animation: string) =>
    reducedMotion ? "opacity-100" : `${animation} opacity-0`;
    ...
    <section className="relative pt-28 pb-16 sm:pt-36 sm:pb-24 px-4 sm:px-6 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-black tracking-tight leading-[1.05] mb-6">
              <span className="block overflow-hidden">
                <span
                  className={cn(
                    "block text-white",
                    entranceClass("animate-slide-up")
                  )}
              ...
              <span className="block overflow-hidden">
                <span
                  className={cn("block", entranceClass("animate-slide-up"))}
                  style={motionStyle(300)}
                >
          ...
          <div
            className={entranceClass("animate-slide-up")}
            style={motionStyle(350)}
          >
            <div className={cn(!reducedMotion && "lg:animate-float")}>
              <ScorePreview />
            </div>
          </div>
        </div>
